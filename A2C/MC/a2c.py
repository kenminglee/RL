...
class Agent(base_agent):
    def __init__(self, env, gl_agent, gl_lock, semaphore, gl_count, num_processes, reward_decay=0.9):
    ...
    def perform_learning_iter(self):
        ...
        loss = torch.sum(-log_probs*(delta.detach())) + 0.1*torch.mean(delta**2)
...
if __name__ == "__main__":
    ...
    for i in range(num_processes):
        p = mp.Process(target=worker, args=(
            gl_agent, gl_r_per_eps, gl_solved, gl_lock, gl_print_lock, semaphore, gl_count, num_processes, i))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
