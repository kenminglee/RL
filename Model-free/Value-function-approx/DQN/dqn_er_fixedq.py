...
class dqn_agent(base_agent):
    def __init__(self, state_dim, actions, experience_replay_size=10000, e_greedy=0.1, learning_rate=0.001, reward_decay=0.9):
        ...
        self.dqn_target = DQN(state_dim, len(actions)).to(device=self.device).double()
        self.dqn_policy = DQN(state_dim, len(actions)).to(device=self.device).double()
    ...
    def choose_action(self, s) -> int:
        if np.random.uniform() >= self.epsilon:
            ...
            with torch.no_grad():
                action_values = self.dqn_policy(torch.tensor(s, dtype=torch.double, device=self.device))
    ...
    def perform_learning_iter(self):
        ...
        next_states_non_terminal_mask = torch.tensor([i is not None for i in next_states], dtype=torch.bool).to(device=self.device)
        states = torch.tensor(states, dtype=torch.double).to(device=self.device)
        actions = torch.tensor(actions).to(device=self.device)
        rewards = torch.tensor(rewards).double().to(device=self.device)
        non_terminal_next_states = torch.tensor([i for i in next_states if i is not None], dtype=torch.double).to(device=self.device)
        ...
        predicted_values = self.dqn_policy(states).gather(1, actions.unsqueeze(1)).squeeze(1)
        
        next_states_values = torch.zeros(rewards.size(), dtype=torch.double).to(device=self.device)
        ...
        with torch.no_grad():
            ...
            next_states_values[next_states_non_terminal_mask] = self.dqn_target(non_terminal_next_states).max(1)[0]
...
if __name__ == "__main__":
    ...
    agent = dqn_agent(env.observation_space.shape[0], [i for i in range(env.action_space.n)])
