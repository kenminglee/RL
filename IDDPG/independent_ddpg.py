...
def run_experiment(display_plot=True, plot_name=None):
    # env = gym.make("MountainCarContinuous-v0")
    env = multiwalker_v6.env()
    agents = {agent_id:Agent(env.observation_spaces[agent_id].shape[0], env.action_spaces[agent_id].shape[0], env.action_spaces[agent_id].high[0]) for agent_id in env.possible_agents}
    r_per_eps = []
    mean_per_100_eps = []
    solved = False
    eps = 0
    while not solved:
        ...
        for agent_id in env.agent_iter():
            a = agents[agent_id].choose_action(o) 
            env.step(a)
            o_, r, done, _ = env.last()
            total_r_in_eps += r
            agents[agent_id].learn(o, a, r, o_, done)
            o = o_
            if done: break
        ...
        if len(r_per_eps)%100==0: 
            ...
            if mean>=200:
                print(f'Solved* Multiwalker in episode {eps}')
                solved = True
        # break
